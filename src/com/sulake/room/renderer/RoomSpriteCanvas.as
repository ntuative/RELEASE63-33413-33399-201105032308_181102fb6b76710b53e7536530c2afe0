package com.sulake.room.renderer
{
   ...
   public class RoomSpriteCanvas implements IRoomRenderingCanvas
   {
      ...
      private static const const_369:int = 50;
      
      private static const const_1452:int = 50;
      
      private static const const_1050:Number = 60;
      
      private static const const_1451:Number = 50;
      ...
      private var var_656:RoomObjectCache;
      
      private var var_531:Array;
      
      private var var_657:Array;
      
      private var var_1520:IRoomRenderingCanvasMouseListener = null;
      ...
      private var var_2719:Number = -1.0E7;
      
      private var var_2724:Number = -1.0E7;
      ...
      public function RoomSpriteCanvas(param1:IRoomSpriteCanvasContainer, param2:String, param3:int, param4:int, param5:int)
      {
         ...
         this.var_265.addChild(this.var_99);
         ...
         this.var_656 = new RoomObjectCache(_loc6_);
         this.var_837 = new ColorTransform();
         this.var_659 = new Matrix();
      ...
      {
         this.var_529.x -= param1 - this.var_660;
         this.var_660 = param1;
      }
      
      public function set screenOffsetY(param1:int) : void
      {
         this.var_529.y -= param1 - this.var_661;
         this.var_661 = param1;
      }
      ...
      {
         this.var_656.removeObjectCache(param1);
      }
      
      public function render(param1:int) : void
      {
         this.var_1959 = !this.var_1959;
         if(this._container == null || this._geometry == null)
         {
            return;
         }
         if(param1 == this.var_1519)
         {
            return;
         }
         ...
         if(this.var_528 != this.var_2722 || this.var_530 != this.var_2721)
         {
            _loc8_ = true;
         }
         if(this.var_99.x != this.var_660 || this.var_99.y != this.var_661)
         {
            this.var_99.x = this.var_660;
            this.var_99.y = this.var_661;
            _loc8_ = true;
         }
         _loc4_ = 0;
         while(_loc4_ < _loc3_)
         ...
         this.var_2722 = this.var_528;
         this.var_2721 = this.var_530;
      }
      ...
      private function calculateUpdateInterval(param1:int) : void
      {
         ...
         if(this.var_1519 > 0)
         {
            ...
            if(_loc2_ > const_1050 * 3)
            {
               Logger.log("Really slow frame update " + _loc2_ + "ms");
               this.var_2718 = _loc2_;
            }
            if(_loc2_ <= MAXIMUM_VALID_FRAME_UPDATE_INTERVAL)
            {
               ++this.var_808;
               if(this.var_808 == const_369 + 1)
               {
                  this.var_510 = _loc2_;
                  this.var_1961 = this.var_2723;
               }
               else if(this.var_808 > const_369 + 1)
               {
                  _loc3_ = Number(this.var_808 - const_369);
                  this.var_510 = this.var_510 * (_loc3_ - 1) / _loc3_ + Number(_loc2_) / _loc3_;
                  this.var_1961 = this.var_1961 * (_loc3_ - 1) / _loc3_ + Number(this.var_2723) / _loc3_;
                  if(this.var_808 > const_369 + const_1452)
                  {
                     this.var_808 = const_369;
                     if(!this.var_1241 && this.var_510 > const_1050)
                     {
                        this.var_1241 = true;
                        Logger.log("Room canvas updating really slow - now entering frame skipping mode...");
                     }
                     else if(this.var_1241 && this.var_510 < const_1451)
                     {
                        this.var_1241 = false;
      ...
      private function renderObject(param1:IRoomObject, param2:String, param3:int, param4:Boolean, param5:int) : int
      {
         ...
         if(_loc6_ == null)
         {
            this.var_656.removeObjectCache(param2);
            return 0;
         }
         var _loc7_:RoomObjectCacheItem = this.var_656.getObjectCache(param2);
         ...
         _loc6_.update(this._geometry,param3,!_loc9_.isEmpty || param4,this.var_1959 && this.var_1241);
      ...
      {
         if(param1 < this.var_528 && param1 + param3 >= 0 && (param2 < this.var_530 && param2 + param4 >= 0))
         {
            if(!this.var_2968)
            {
               return true;
      ...
      private function rectangleVisibleWithExclusion(param1:int, param2:int, param3:int, param4:int) : Boolean
      {
         ...
         for each(_loc5_ in this.var_2720)
         {
      ...
      {
         if(param1 < 0 || param1 >= this.var_1005)
         {
            return null;
         }
      ...
      private function createSprite(param1:SortableSprite, param2:int = -1) : void
      {
         ...
         if(param2 < 0 || param2 >= this.var_1005)
         {
      ...
      private function getFlippedBitmapData(param1:BitmapData, param2:String, param3:Boolean = false, param4:Boolean = true, param5:Boolean = false) : ExtendedBitmapData
      {
         ...
         if(finalData == null)
         {
            ...
            this.var_659.identity();
            if(flipH)
            {
               this.var_659.scale(-1,1);
               this.var_659.translate(data.width,0);
            }
            if(flipV)
            {
               this.var_659.scale(1,-1);
               this.var_659.translate(0,data.height);
            }
            finalData.draw(data,this.var_659);
            if(name.length > 0)
            {
      ...
      private function getColoredBitmapData(param1:BitmapData, param2:String, param3:int, param4:Boolean = false) : ExtendedBitmapData
      {
         ...
         if(finalData == null)
         {
            ...
            this.var_837.redMultiplier = tR;
            this.var_837.greenMultiplier = tG;
            this.var_837.blueMultiplier = tB;
            finalData.colorTransform(finalData.rect,this.var_837);
            if(name.length > 0)
            {
               this.var_266.addBitmapData(cacheName,finalData);
            }
      ...
      public function handleMouseEvent(param1:int, param2:int, param3:String, param4:Boolean, param5:Boolean, param6:Boolean, param7:Boolean) : Boolean
      {
         ...
         if(this.var_1240 > 0 && param3 == MouseEvent.MOUSE_MOVE)
         {
            return this.var_1960;
         }
         this.var_1960 = this.checkMouseHits(param1,param2,param3,param4,param5,param6,param7);
         ++this.var_1240;
         return this.var_1960;
      }
      ...
      private function createMouseEvent(param1:int, param2:int, param3:int, param4:int, param5:String, param6:String, param7:Boolean, param8:Boolean, param9:Boolean, param10:Boolean) : RoomSpriteMouseEvent
      {
         ...
         var _loc13_:* = null;
         return new RoomSpriteMouseEvent(param5,this._id + "_" + this.var_1958,this._id,param6,_loc11_,_loc12_,param3,param4,param8,param7,param9,param10);
      }
            ...
            if(_loc10_ != null && _loc10_.hitTestPoint(param1 - _loc10_.x,param2 - _loc10_.y))
            {
               if(!_loc10_.clickHandling || param3 != MouseEvent.CLICK && param3 != MouseEvent.DOUBLE_CLICK)
               {
                  ...
                  if(_loc12_.indexOf(_loc9_) < 0)
                  {
                     ...
                     if(param3 != MouseEvent.MOUSE_MOVE || param1 != this.var_2719 || param2 != this.var_2724)
                     {
                        this.bufferMouseEvent(_loc11_,_loc9_);
                     }
                     _loc12_.push(_loc9_);
      ...
      {
         if(this.var_379 != null && param1 != null)
         {
            this.var_379.add(param2,param1);
         }
      }
      ...
      private function processMouseEvents() : void
      {
         ...
         var _loc4_:* = null;
         var _loc5_:* = null;
         if(this._container == null || this.var_379 == null)
         {
            return;
         }
         var _loc1_:int = 0;
         ...
         {
            _loc2_ = this.var_379.getKey(_loc1_);
            _loc3_ = this.var_379.getWithIndex(_loc1_);
            if(_loc2_ != null && _loc3_ != null)
            {
               ...
               if(_loc4_ != null)
               {
                  ...
                  {
                     this.var_1520.processRoomCanvasMouseEvent(_loc3_,_loc4_,this.geometry);
                  }
                  else
                  {
