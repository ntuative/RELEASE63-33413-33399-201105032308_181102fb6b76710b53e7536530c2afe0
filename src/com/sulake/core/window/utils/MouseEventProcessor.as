package com.sulake.core.window.utils
{
   ...
   public class MouseEventProcessor implements IEventProcessor, IDisposable
   {
      ...
      public function MouseEventProcessor()
      {
         ...
         {
            var_335 = new Array();
            var_335[0] = MouseCursorType.const_364;
            var_335[1] = MouseCursorType.const_30;
            var_335[2] = MouseCursorType.const_364;
            var_335[3] = MouseCursorType.const_364;
            var_335[4] = MouseCursorType.const_364;
            var_335[5] = MouseCursorType.const_30;
            var_335[6] = MouseCursorType.const_364;
         }
         if(var_288 == null)
         {
            var_288 = new Array();
            var_288[0] = WindowState.const_90;
            var_288[1] = WindowState.const_88;
            var_288[2] = WindowState.const_85;
            var_288[3] = WindowState.const_73;
            var_288[4] = WindowState.const_53;
            var_288[5] = WindowState.const_93;
      ...
      public function process(param1:EventProcessorState, param2:IEventQueue) : void
      {
         ...
         this.var_62 = state.var_1328 as WindowController;
         this.var_175 = state.var_1329 as WindowController;
         this.var_172 = state.renderer;
         this.var_876 = state.var_1330;
         eventQueue.begin();
         this.var_708.x = -1;
         this.var_708.y = -1;
         ...
         while(true)
         {
            ...
            if(event.stageX != this.var_708.x || event.stageY != this.var_708.y)
            {
               ...
               array = new Array();
               this.var_81.groupParameterFilteredChildrenUnderPoint(this.var_708,array,WindowParam.const_31);
            }
            ...
            if(index == 0)
            {
               if(event.type == MouseEvent.MOUSE_MOVE)
               {
                  ...
                  {
                     this.var_62.getGlobalPosition(var_407);
               ...
               if(child != null && child.visible)
               {
                  ...
                  else if(event.type == MouseEvent.MOUSE_UP || event.type == MouseEvent.CLICK)
                  {
                     if(this.var_62 && !this.var_62.disposed)
                     {
                        ...
                        {
                           this.var_876.mouseEventReceived(event.type,this.var_62);
                        }
                     }
                  }
                  temp = child.parent;
         ...
         state.var_1328 = this.var_62;
         state.var_1329 = this.var_175;
         state.renderer = this.var_172;
         state.var_1330 = this.var_876;
      }
