package com.sulake.core.window.graphics
{
   ...
   public class WindowRenderer implements IWindowRenderer
   {
      ...
      public function WindowRenderer(param1:ISkinContainer)
      {
         ...
         this.var_1949 = new Point();
         this.var_1516 = new Rectangle();
         this.var_313 = new Rectangle();
      ...
      public function addToRenderQueue(param1:IWindow, param2:Rectangle, param3:uint) : void
      {
         ...
         if(this.getWindowRendererItem(param1).invalidate(param1,param2,param3))
         {
            ...
            this.var_313.x = param2.x - param1.x;
            this.var_313.y = param2.y - param1.y;
            this.var_313.width = param2.width;
            this.var_313.height = param2.height;
            if(param1.testParamFlag(WindowParam.const_34))
            {
               while(param1.testParamFlag(WindowParam.const_34))
               {
                  ...
                  this.var_313.offset(param1.x,param1.y);
                  param1 = _loc4_;
               }
            }
            this.getWindowRendererItem(param1).invalidate(param1,this.var_313,WindowRedrawFlag.const_1243);
      ...
      public function registerRenderable(param1:IWindow) : void
      {
         ...
         if(_loc2_ == null)
         {
            ...
            param1.addEventListener(WindowDisposeEvent.const_816,this.windowDisposedCallback);
         }
      }
      
      public function removeRenderable(param1:IWindow) : void
      {
         param1.removeEventListener(WindowDisposeEvent.const_816,this.windowDisposedCallback);
         var _loc2_:WindowRendererItem = this.var_375[param1] as WindowRendererItem;
         if(_loc2_ != null)
         {
            _loc2_.dispose();
      ...
      public function render() : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            ...
            if(!_loc1_.disposed)
            {
               ...
               this.var_313.x = _loc3_.dirty.x;
               this.var_313.y = _loc3_.dirty.y;
               this.var_313.width = _loc3_.dirty.width;
               this.var_313.height = _loc3_.dirty.height;
               this.renderWindowBranch(_loc1_,this.var_313,_loc1_.rectangle.clone());
            }
            _loc4_++;
         }
      }
      ...
      private function renderWindowBranch(param1:WindowController, param2:Rectangle, param3:Rectangle) : void
      {
         ...
         if(param1.visible)
         {
            if(getDrawLocationAndClipRegion(param1,param2,this.var_1949,this.var_1516))
            {
               if(this.getWindowRendererItem(param1).render(param1,this.var_1949,this.var_1516))
               {
            ...
            else if(!param1.testParamFlag(WindowParam.const_34))
            {
               ...
               {
                  param1.getGraphicContext(true).setDrawRegion(param1.rectangle,false,this.var_1516);
