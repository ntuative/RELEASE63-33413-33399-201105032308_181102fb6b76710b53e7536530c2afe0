package com.sulake.habbo.messenger.domain
{
   ...
   public class Conversations
   {
      ...
      private static var var_1155:int = 1;
      ...
      public function setSelected(param1:Conversation) : void
      {
         ...
         var _loc2_:int = this.var_135.indexOf(param1);
      ...
      public function closeConversation() : void
      {
         ...
         if(_loc1_ != null)
         {
            _loc2_ = this.var_135.indexOf(_loc1_);
            Util.remove(this.var_135,_loc1_);
            this._closedConversations.push(_loc1_);
            _loc1_.setSelected(false);
            while(_loc2_ >= 0)
            {
               if(this.var_135[_loc2_] != null)
               {
                  this.var_135[_loc2_].setSelected(true);
                  break;
               }
               _loc2_--;
            }
      ...
      {
         this.addConversation(var_1155++);
      }
      
      public function addConversation(param1:int) : Conversation
      {
         var _loc2_:Boolean = this._closedConversations.length == 0 && this.var_135.length == 0;
         var _loc3_:Conversation = this.addConversationInt(param1);
         if(_loc3_ == null)
         {
            return null;
         }
         if(this.var_135.length == 1)
         {
            this.var_135[0].setSelected(true);
         }
         if(_loc2_)
         {
            _loc3_.addMessage(new Message(Message.const_920,0,this.var_429.getText("messenger.moderationinfo"),""));
         }
         return _loc3_;
      }
      ...
      {
         var _loc2_:int = this.var_135.length;
         ...
         if(_loc2_ != this.var_135.length || _loc4_ != _loc3_.newMessageArrived)
         {
      ...
      {
         var _loc1_:* = null;
         for each(_loc1_ in this.var_135)
         {
            if(_loc1_.selected)
            {
               return _loc1_;
      ...
      {
         this._startIndex = Math.min(this._startIndex,this.var_135.length - this.var_429.getTabCount());
         this._startIndex = Math.max(0,this._startIndex);
      }
      
      private function getOnlineInfoMsg(param1:Boolean) : Message
      {
         return new Message(Message.const_738,0,this.getOnlineText(param1),Util.getFormattedNow());
      }
