package com.sulake.habbo.sound.music
{
   ...
   public class JukeboxPlayListController implements IPlayListController
   {
      ...
      public function JukeboxPlayListController(param1:HabboSoundManagerFlash10, param2:HabboMusicController, param3:IEventDispatcher, param4:IConnection)
      {
         ...
         this.var_713 = [];
         this.var_713.push(new NowPlayingMessageEvent(this.onNowPlayingMessage));
         this.var_713.push(new JukeboxSongDisksMessageEvent(this.onJukeboxSongDisksMessage));
         this.var_713.push(new JukeboxPlayListFullMessageEvent(this.onJukeboxPlayListFullMessage));
      ...
      {
         if(param1 < 0 || param1 >= this.var_483.length)
         {
            return null;
         }
      ...
      private function onNowPlayingMessage(param1:IMessageEvent) : void
      {
         ...
         this.var_668 = _loc3_.currentSongId != -1;
         if(_loc3_.currentSongId >= 0)
         {
            this.var_231.playSong(_loc3_.currentSongId,HabboMusicPrioritiesEnum.const_120,_loc3_.syncCount / 1000,0,1,1);
            this.var_445 = _loc3_.currentSongId;
         }
         else
         {
            this.stopPlaying();
         ...
         this.var_1637 = _loc3_.currentPosition;
      ...
      private function onJukeboxSongDisksMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDisks.length)
         {
            ...
            if(_loc7_ == null)
            {
               ...
               if(this.var_714.indexOf(_loc5_) < 0)
               {
         ...
         while(_loc2_ < this.length)
         {
            ...
            if(_loc4_.id == param1.id)
            {
               ...
               if(_loc6_ != null)
               {
                  ...
                  this.var_483[_loc2_] = _loc6_;
               }
               break;
            }
            _loc2_++;
         }
         var _loc3_:int = this.var_714.indexOf(param1.id);
         if(_loc3_ >= 0)
         {
            this.var_714.splice(_loc3_,1);
         }
         if(this.var_714.length == 0)
         {
            this._events.dispatchEvent(new PlayListStatusEvent(PlayListStatusEvent.const_104));
