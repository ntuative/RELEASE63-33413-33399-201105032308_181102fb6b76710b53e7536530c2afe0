package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private function createView() : void
      {
         ...
         this.var_374 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_652 && this.var_1515)
         {
            _loc5_ = this._controller.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_99,this.var_1003,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_35,parseInt(FigureData.const_878));
         ...
         if(this.var_374)
         {
            if(this.var_374.bitmap)
            {
               this.var_374.bitmap.dispose();
            }
            this.var_374.bitmap = new BitmapData(this.var_374.width,this.var_374.height,true,0);
            _loc7_ = (this.var_374.width - _loc1_.width) / 2;
            _loc8_ = (this.var_374.height - _loc1_.height) / 2;
            this.var_374.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               this.var_652 = this._controller.figureData.getFigureString();
               this.var_1003 = this._controller.gender;
               this._controller.handler.saveWardrobeOutfit(this.var_1946,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_652)
               {
                  this._controller.loadAvatarInEditor(this.var_652,this.var_1003,this._controller.clubMemberLevel);
               }
         }
      }
