package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_934 = new Timer(this.const_2133,1);
         this.var_934.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_932);
            if(this.var_1420.indexOf("?") > 0)
            {
               _loc2_ = this.var_1420 + "&retry=" + this.var_932;
            }
            else
            {
               _loc2_ = this.var_1420 + "?retry=" + this.var_932;
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1785 = true;
         for each(_loc2_ in this.var_1165)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2417,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1164[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1164[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1785)
         {
            this.var_1165.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_509[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_509[_loc3_] = _loc6_;
            }
      ...
      {
         var _loc1_:* = null;
         while(this.var_752.length > 0 && this.var_753.length < this.const_1603)
         {
            _loc1_ = this.var_752[0];
            _loc1_.startDownloading();
            this.var_753.push(this.var_752.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_752.indexOf(param1) == -1 && this.var_753.indexOf(param1) == -1)
         {
