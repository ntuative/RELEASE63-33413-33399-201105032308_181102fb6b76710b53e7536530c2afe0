package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_738:IFigurePartSet;
      ...
      private const const_2120:int = 13421772;
      
      private const const_2119:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_738 = param3;
         this._window = param1;
         this.var_1400 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1748 = Math.max(this.var_1748,_loc6_.colorLayerIndex);
            }
         }
         this.var_426 = this.var_67.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_700 != null && !this.var_1746)
            {
               _loc3_ = this.var_700;
            }
            else
            ...
            if(this.var_738)
            {
               switch(this.var_738.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_46:
                     _loc2_.style = HabboIconType.const_943;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1400.color = this.const_2120;
         }
         else
         {
            this.var_1400.color = this.const_2119;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_426.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_426.isFigureReady(_loc1_))
         {
            this.var_426.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_367[_loc6_] + "_" + FigureData.const_138;
               _loc4_ = this.var_426.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         _loc1_ = new BitmapData(this.var_612.width,this.var_612.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_612.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_612.y;
