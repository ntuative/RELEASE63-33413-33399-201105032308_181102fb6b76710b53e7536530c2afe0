package com.sulake.habbo.room.utils
{
   import com.sulake.room.utils.IVector3d;
   import com.sulake.room.utils.Vector3d;
   
   public class RoomCamera
   {
      
      private static const const_1435:Number = 12;
       
      
      private var var_2651:int = -1;
      
      private var var_2650:int = -2;
      
      private var var_211:Vector3d = null;
      
      private var var_1224:Number = 0;
      
      private var var_1496:Number = 0;
      
      private var var_1923:Boolean = false;
      
      private var var_189:Vector3d = null;
      
      private var var_1922:Vector3d;
      
      private var var_2648:Boolean = false;
      
      private var var_2652:Boolean = false;
      
      private var var_2644:Boolean = false;
      
      private var var_2649:Boolean = false;
      
      private var var_2645:int = 0;
      
      private var var_2647:int = 0;
      
      private var _scale:int = 0;
      
      private var var_2646:int = 0;
      
      private var var_2643:int = 0;
      
      private var var_1806:int = -1;
      
      private var var_1921:int = 0;
      
      private var var_1920:Boolean = false;
      
      public function RoomCamera()
      {
         this.var_1922 = new Vector3d();
         super();
      }
      
      public function get location() : IVector3d
      {
         return this.var_189;
      }
      
      public function get targetId() : int
      {
         return this.var_2651;
      }
      
      public function get targetCategory() : int
      {
         return this.var_2650;
      }
      
      public function get targetObjectLoc() : IVector3d
      {
         return this.var_1922;
      }
      
      public function get limitedLocationX() : Boolean
      {
         return this.var_2648;
      }
      
      public function get limitedLocationY() : Boolean
      {
         return this.var_2652;
      }
      
      public function get centeredLocX() : Boolean
      {
         return this.var_2644;
      }
      
      public function get centeredLocY() : Boolean
      {
         return this.var_2649;
      }
      
      public function get screenWd() : int
      {
         return this.var_2645;
      }
      
      public function get screenHt() : int
      {
         return this.var_2647;
      }
      
      public function get scale() : int
      {
         return this._scale;
      }
      
      public function get roomWd() : int
      {
         return this.var_2646;
      }
      
      public function get roomHt() : int
      {
         return this.var_2643;
      }
      
      public function get geometryUpdateId() : int
      {
         return this.var_1806;
      }
      
      public function get isMoving() : Boolean
      {
         if(this.var_211 != null && this.var_189 != null)
         {
            return true;
         }
         return false;
      }
      
      public function set targetId(param1:int) : void
      {
         this.var_2651 = param1;
      }
      
      public function set targetObjectLoc(param1:IVector3d) : void
      {
         this.var_1922.assign(param1);
      }
      
      public function set targetCategory(param1:int) : void
      {
         this.var_2650 = param1;
      }
      
      public function set limitedLocationX(param1:Boolean) : void
      {
         this.var_2648 = param1;
      }
      
      public function set limitedLocationY(param1:Boolean) : void
      {
         this.var_2652 = param1;
      }
      
      public function set centeredLocX(param1:Boolean) : void
      {
         this.var_2644 = param1;
      }
      
      public function set centeredLocY(param1:Boolean) : void
      {
         this.var_2649 = param1;
      }
      
      public function set screenWd(param1:int) : void
      {
         this.var_2645 = param1;
      }
      
      public function set screenHt(param1:int) : void
      {
         this.var_2647 = param1;
      }
      
      public function set scale(param1:int) : void
      {
         if(this._scale != param1)
         {
            this._scale = param1;
            this.var_1920 = true;
         }
      }
      
      public function set roomWd(param1:int) : void
      {
         this.var_2646 = param1;
      }
      
      public function set roomHt(param1:int) : void
      {
         this.var_2643 = param1;
      }
      
      public function set geometryUpdateId(param1:int) : void
      {
         this.var_1806 = param1;
      }
      
      public function set target(param1:IVector3d) : void
      {
         var _loc2_:* = null;
         if(this.var_211 == null)
         {
            this.var_211 = new Vector3d();
         }
         if(this.var_211.x != param1.x || this.var_211.y != param1.y || this.var_211.z != param1.z)
         {
            this.var_211.assign(param1);
            this.var_1921 = 0;
            _loc2_ = Vector3d.dif(this.var_211,this.var_189);
            this.var_1224 = _loc2_.length;
            this.var_1923 = true;
         }
      }
      
      public function dispose() : void
      {
         this.var_211 = null;
         this.var_189 = null;
      }
      
      public function initializeLocation(param1:IVector3d) : void
      {
         if(this.var_189 != null)
         {
            return;
         }
         this.var_189 = new Vector3d();
         this.var_189.assign(param1);
      }
      
      public function resetLocation(param1:IVector3d) : void
      {
         if(this.var_189 == null)
         {
            this.var_189 = new Vector3d();
         }
         this.var_189.assign(param1);
      }
      
      public function update(param1:uint, param2:Number) : void
      {
         var _loc3_:* = null;
         var _loc4_:Number = NaN;
         var _loc5_:Number = NaN;
         var _loc6_:Number = NaN;
         var _loc7_:Number = NaN;
         if(this.var_211 != null && this.var_189 != null)
         {
            ++this.var_1921;
            if(this.var_1920)
            {
               this.var_1920 = false;
               this.var_189 = this.var_211;
               this.var_211 = null;
               return;
            }
            _loc3_ = Vector3d.dif(this.var_211,this.var_189);
            if(_loc3_.length > this.var_1224)
            {
               this.var_1224 = _loc3_.length;
            }
            if(_loc3_.length <= param2)
            {
               this.var_189 = this.var_211;
               this.var_211 = null;
               this.var_1496 = 0;
            }
            else
            {
               _loc4_ = Math.sin(0 * _loc3_.length / this.var_1224);
               _loc5_ = param2 * 0.5;
               _loc6_ = this.var_1224 / const_1435;
               _loc7_ = _loc5_ + (_loc6_ - _loc5_) * _loc4_;
               if(this.var_1923)
               {
                  if(_loc7_ < this.var_1496)
                  {
                     _loc7_ = this.var_1496;
                     if(_loc7_ > _loc3_.length)
                     {
                        _loc7_ = _loc3_.length;
                     }
                  }
                  else
                  {
                     this.var_1923 = false;
                  }
               }
               this.var_1496 = _loc7_;
               _loc3_.div(_loc3_.length);
               _loc3_.mul(_loc7_);
               this.var_189 = Vector3d.sum(this.var_189,_loc3_);
            }
         }
      }
   }
}
