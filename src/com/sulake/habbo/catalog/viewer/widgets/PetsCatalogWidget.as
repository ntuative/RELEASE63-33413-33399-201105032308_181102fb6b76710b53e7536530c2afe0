package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_2288 = _loc3_.localizationId;
         this.var_377 = [];
         if(page && false && page.viewer.catalog)
         {
         ...
         events.addEventListener(WidgetEvent.const_739,this.onApproveNameResult);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_377)
         {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_526 = param1.index;
         if(this.var_526 < 0 || this.var_526 > this.var_377.length)
         {
            this.var_526 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.productCode == this.var_2288)
         {
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_212 == null || this.var_806 >= this.var_212.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_526 >= 0 && this.var_526 < this.var_377.length)
         {
            _loc7_ = this.var_377[this.var_526] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_212[this.var_806] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_493,_loc3_,_loc2_,AvatarScaleType.const_56,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_323,AvatarAction.const_779);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_526 >= this.var_377.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_377[this.var_526] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_212[this.var_806] as int;
